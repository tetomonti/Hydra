...
def submit_jobs(index, param, py_file, job_id, cores, environment):
    ...
    if environment == 'modules_python2.7':
        #Loading the python2.7 module is already included in the parameter list under python2.7 environment variable
        #Unload python/2.7.5 module in case it was loaded last to avoud environment conflicts
        env=list()
        command_list[:0] = ['module unload python/2.7.5']
        cmd='python2.7 '
    elif environment == 'modules_python2.7.5':
        env=list()
        command_list[:0] = ['module unload python2.7/Python-2.7.3_gnu446']
        cmd='python2.7 '    
    else:
        #if we need to run python2.6 (or python/2.7.5) just unload python2.7, otherwise it results in errors
        command_list[:0] = ['module unload python2.7/Python-2.7.3_gnu446']
        cmd='python '
