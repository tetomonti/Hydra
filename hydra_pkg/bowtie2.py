...
def main():
    ...
    #convert output to bam file
    call.append('|')
    call.append('samtools')
    call.append('view')
    call.append('-bS')
    call.append('-')
    call.append('>')
    call.append(outdir + 'accepted_hits.bam')
    ...
    #error handling - checking if bowtie logfile exists and if it does,
    #check if bowtie2 was run successful
    bowtie2_logfile = outdir + 'logs/bowtie2.log'

    if not os.path.exists(bowtie2_logfile):
        sys.exit(0)
    else:
        log = open(bowtie2_logfile)
