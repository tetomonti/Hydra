 ...
 )
{
  ...
  if ( is.null(cls) )
  {
    ...
    cls.perm <- permute.paired(cls,n=nperm,exhaustive=exhaustive,balanced=balanced)
  }
  else {
    cls.perm <- permute.binarray.new( cls, n=nperm, control=control, exhaustive=exhaustive,
  ...
  if ( do.debug )
    return( list(sum=x.out,prm=x.prm,obs=x.obs) )
  else
    return( x.out )
}
# FUNCTION: KS WRAPPER
